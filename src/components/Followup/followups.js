...
import FilterForm from "./filter_modal";
...
class Followups extends React.Component {
  ...
  componentDidMount() {
    console.log("followups mounted");
    this.action();
  }
  render() {
    return (
      <View style={{ flex: 1, backgroundColor: "white" }}>
        <Header title="Followups" navigation={this.props.navigation}>
        ...
        {this.state.followup ? (
          this.state.filterForm ? (
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <DataLayout data={item} append={true} />
              )}
